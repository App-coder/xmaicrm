...
function edit() {
    ...
    if (selected) {
	window.location.href = "crm/module/documents/showedit?recordid="
		+ selected.documentsid + "&ptb=" + ptb + "&module="
		+ entitytype;
    } else {
	message("请选择记录！");
    }
...
function view() {
    var selected = $('#view_list').datagrid("getSelected");
    if (selected) {
	window.location.href = "crm/module/documents/view?recordid="
		+ selected.documentsid + "&ptb=" + ptb + "&module="
		+ entitytype;
    } else {
	message("请选择记录！");
...
function deleteRecord() {
	...
	confirm('确定删除文档？', function(r) {
		...
		$.post('crm/module/documents/deleteRecords', {
		    recordids : recordids
		}, function(res) {
		    if (res.type == true) {
